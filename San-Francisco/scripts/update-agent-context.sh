...
update_agent_file() {
    ...
    if [ ! -f "$target_file" ]; then
        ...
        if [[ "$NEW_LANG" == *"Python"* ]]; then
            ...
            COMMANDS="echo 'JavaScript/TypeScript commands removed - focusing on PHP implementation'"
        else
...
if "$NEW_LANG" and f"# {NEW_LANG}" not in content:
    ...
    if commands_section:
        ...
        if "Python" in "$NEW_LANG":
            ...
            new_commands += "\necho 'JavaScript/TypeScript commands removed - focusing on PHP implementation'"
